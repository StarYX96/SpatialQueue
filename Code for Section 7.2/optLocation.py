...
if __name__ == '__main__':
    ...
    distance = distance.drop(columns=['X6', 'X7', 'X8'])
    ...
    threshold = 7.5
    Table = pd.DataFrame(columns=['location', 'prob_BnD', 'utilization', 'fracDist', 'MRT', 'frac_total_%s' % threshold])

    location = itertools.combinations(np.array(list(range(distance.shape[1]))), N)
    alt = 0
    for alternative in location:
        t = np.array(distance)[:, alternative]
        Pre_L = np.zeros((J, N))
        for i in range(J):
        ...
        transup, transdown = Transition_Rate(N, J, Lambda, Mu, Pre_L, f)
        ...
        MRT_BnD, thresholdFrac = MRT(N, J, prob_dist_BnD, Pre_L, f, t, threshold)

        print("------alternative No. %s: %s seconds ------" % (alt, time.time() - start_time))

        Table.loc[len(Table.index)] = [alternative, prob_dist_BnD[1], utilizeRate, thresholdFrac, MRT_BnD, thresholdFrac[-1]]

        del transup, transdown, prob_dist_BnD, utilizeRate

    Table.to_csv('Table-OptLocation-%sUnits-%sthreshould.csv' % (N, threshold + 1.5), index=None)
