...
# ------------------ 数据加载 ------------------
df = pd.read_csv("homoResults.csv")

# ------------------ 数据预处理 ------------------
# 转换长表
df_long = pd.melt(
    df,
    id_vars=["Location", "rho"],
    value_vars=[f"N={n} ({metric})" for n in range(11, 21) for metric in ["Time Saving", "MPRE"]],
    var_name="Metric",
    value_name="Value"
)
...
# ------------------ 绘制Time Saving对比图 ------------------
# 准备数据
time_saving_st_paul = [st_paul_data[(st_paul_data["Metric"] == "Time Saving") & (st_paul_data["N"] == n)]["Value"].values for n in range(11, 21)]
time_saving_greenville = [greenville_data[(greenville_data["Metric"] == "Time Saving") & (greenville_data["N"] == n)]["Value"].values for n in range(11, 21)]

# 绘制St. Paul箱线图
bp1 = axes[0].boxplot(
    time_saving_st_paul,
    positions=np.arange(11, 21) - position_offset,
    widths=box_width,
    patch_artist=True,
...
axes[0].set_xticks(range(11, 21))
axes[0].set_xticklabels(range(11, 21))
...
# ------------------ 绘制MPRE对比图 ------------------
# 准备数据
mpre_st_paul = [st_paul_data[(st_paul_data["Metric"] == "MPRE") & (st_paul_data["N"] == n)]["Value"].values for n in range(11, 21)]
mpre_greenville = [greenville_data[(greenville_data["Metric"] == "MPRE") & (greenville_data["N"] == n)]["Value"].values for n in range(11, 21)]
...
axes[1].set_xticks(range(11, 21))
axes[1].set_xticklabels(range(11, 21))
axes[1].set_ylim(ylim_config["MPRE"])
for ax in axes:
    ax.tick_params(axis='both', which='major', labelsize=14)
axes[1].legend([bp3["boxes"][0], bp4["boxes"][0]], ["St. Paul", "Greenville"], loc="upper left", fontsize=14)

# ------------------ 保存图片 ------------------
plt.savefig("TimeSaving&MPRE-homo-colored.pdf", dpi=600, bbox_inches="tight")
plt.show()
